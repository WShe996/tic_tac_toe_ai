...
class game_board {
    public:
        game_board() : board_vector{{0, 0, 0}, {0, 0, 0}, {0, 0, 0}}, turn(true) {}
        ...
        std::vector<int> get_game_vector();
...
{
    int player_multiplyer = turn ? -1 : 1;
    
    if (board_vector[0][0] + board_vector[1][1] + board_vector[2][2] == player_multiplyer * 3 || 
        board_vector[2][0] + board_vector[1][1] + board_vector[0][2] ==  player_multiplyer * 3) {
        return true;
    }
      
    for (int i = 0; i < 3; i++) {
        if (board_vector[0][i] + board_vector[1][i] + board_vector[2][i] == player_multiplyer * 3 || 
            board_vector[i][0] + board_vector[i][1] + board_vector[i][2] == player_multiplyer * 3) {
            return true;
        }
    }
...
bool game_board::can_move(int x, int y) {
    if ((x >= 3) || (y >= 3)) {
        return false;
    }
    if (board_vector[x][y] != 0) {
        ...
        board_vector[x][y] = this->turn ? 1 : -1;
       turn = !this->turn;
        return true;
    }
}
...
void game_board::print_board()
{
    for(int i=0; i < 3; i++)
    {
        ...
        {
            char marker;
            if (board_vector[i][j] == 1)
            {
                marker = 'X';
            }
            else if (board_vector[i][j] == 0)
            {
                marker = ' ';
            }
            else
